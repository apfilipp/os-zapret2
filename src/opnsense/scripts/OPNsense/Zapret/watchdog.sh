...
#
# The watchdog itself can be turned off entirely (no control probes at all)
# via the "Enable safety watchdog" option in General Settings (WATCHDOG_ENABLED
# in zapret.conf) or per host via /usr/local/etc/zapret2/watchdog.conf
# (WATCHDOG_ENABLED=0).
...
# Built-in defaults. zapret.conf (generated by the GUI) may set WATCHDOG_ENABLED
# and other values; the manual per-host file watchdog.conf takes precedence.
WATCHDOG_ENABLED="1"
...
# Manual per-host overrides (watchdog.conf) take precedence over the
# GUI-generated zapret.conf.
[ -f "${ZAPRET_DIR}/watchdog.conf" ] && . "${ZAPRET_DIR}/watchdog.conf"

# Bail if the watchdog is disabled (GUI "Enable safety watchdog" unchecked,
# or WATCHDOG_ENABLED=0 in watchdog.conf). No control probes are attempted.
